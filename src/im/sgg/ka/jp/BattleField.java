...
public class BattleField {
    ...
    public boolean isEmpty(int row,int line){
        if (row+1>FIELD_SIZE || line+1>FIELD_SIZE || row<0 || line<0 ||
                scanQuadrant(row, line).trim().isEmpty()) return true;
        return false;
    }
    ...
    public void printField(){
        for (String[] s:this.battleField) System.out.println(Arrays.toString(s).
                replace("[,", "[ ,").replace(" B", "B").replace(",]", ", ]"));
    }

    private boolean isBattleFieldFilled(){
        if (this.battleField==null || this.battleField.length<1 || this.battleField[0]==null) return false;
        else return true;
    }
